...
Create all MVP forms programmatically using `Application.CreateForm` and `Application.CreateControl`. This file contains three VBA modules that must all be in place before running `CreateAllForms`.

## Prerequisites

All of the following modules must already be set up:
- `modTableSetup` — tables must exist (run `CreateAllTables` first)
- `modSeedData` — lookup tables must be seeded (run `SeedAllLookupTables` first)
- `modQuerySetup` — queries must exist (run `CreateAllQueries` first)
- `modLotteryLogic` — business logic module must be pasted
- `modStartup` — startup module must be pasted

## Setup Steps

1. Open `NorrisPowerballPool.accdb` in Microsoft Access.
2. Press **Alt+F11** to open the VBA editor.
3. Create **three** new modules (Insert → Module, three times) and paste the code blocks below into each one.
4. Rename them in the Properties window (**F4**):
   - First module → `modUIConstants`
   - Second module → `modFormEvents`
   - Third module → `modFormSetup`
5. Press **Ctrl+S** to save.
6. Press **Ctrl+G** to open the **Immediate Window**.
7. Type `CreateAllForms` and press **Enter**.
8. A confirmation message appears when all six forms have been created.

---

## VBA Code: `modUIConstants`
...
- **Safe to re-run.** Each form creation sub checks `FormExists` and skips if the form already exists.
- **Error recovery.** If a form fails mid-creation, the error handler attempts to close (without saving) the temp-named form to avoid orphaned design objects.
- **Event binding.** All form events use `=FunctionName()` syntax to call public functions in `modFormEvents`. No form code-behind is needed — all event logic lives in standard modules.
- **frmMainDashboard** calls `=InitializeApp()` on its OnOpen event, which loads global settings from `tblSystemSettings`.
- **frmMatchResults** uses a listbox whose RowSource references `Forms!frmMatchResults!cboDrawing` to filter results by the selected drawing. The `Nz(..., 0)` wrapper returns no results when no drawing is selected.
- **White ball uniqueness** is validated by `modLotteryLogic.ValidateTicketNumbers`. Table-level field validation catches out-of-range values. Form-level BeforeUpdate validation (which requires code-behind) can be added in a future phase by Daniel.
- **Layout tweaking.** To adjust control positions after creation, open any form in Design View and drag controls. The programmatic coordinates provide a clean starting layout.
